// src/components/Layout/UpasLayoutSimple.js - Version sans dashboard
...
const UpasLayoutSimple = ({ children }) => {
  ...
  // Menu items MODIFIÉS - Dashboard retiré
  const menuItems = [
  ...
  // Fonctions MODIFIÉES pour la navigation
  const getPageTitle = () => {
  ...
  // Breadcrumbs MODIFIÉS - pas de dashboard
  const generateBreadcrumbs = () => {
    const pathname = location.pathname;
    const breadcrumbs = [];
    
    // Plus de breadcrumb "Tableau de bord" - on commence directement avec la page actuelle
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      >
        <Toolbar>
          ...
          {/* Titre avec breadcrumbs MODIFIÉS */}
          <Box sx={{ flexGrow: 1 }}>
